...
export default function InvestmentsTab({ plans, profile, onBuyPlan }: InvestmentsTabProps) {
  ...
  return (
    <div className="space-y-5 pb-20 animate-in fade-in slide-in-from-bottom-4 duration-300">
      ...
      {/* Starter Plans Section */}
      <div className="mb-8 space-y-4">
        <div className="flex items-center space-x-2 pb-1.5 border-b border-slate-200">
          <Sparkles className="w-5 h-5 text-amber-500 animate-pulse animate-duration-2000" />
          <h2 className="text-sm font-black text-[#0A3D91] uppercase tracking-wider">
            {language === 'am' ? 'ጀማሪ የኢንቨስትመንት ደረጃዎች (Starter Tiers)' : 'Starter Investment Levels'}
          </h2>
        </div>
        <p className="text-[11px] text-slate-500 font-bold -mt-2">
          {language === 'am' ? 'ምንም የማንነት ማረጋገጫ (KYC) ሳያስፈልግዎት አሁኑኑ ኢንቨስት ያድርጉ።' : 'Identity verification is not required for any Starter Level.'}
        </p>
              ...
              >
                <div>
                  <div className="flex items-center justify-between">
                    <span className="px-3 py-1 text-[10px] font-black rounded-lg bg-gradient-to-r from-[#0A3D91] to-blue-700 text-white uppercase tracking-wider shadow-2xs font-mono">
                      {p.name}
                    </span>
                  ...
                  <div className="mt-3.5 bg-blue-50/50 rounded-xl p-2.5 px-3 flex justify-between items-center text-[11px] border-2 border-blue-150" key={getPlanDuration(p.level)}>
                ...
                <button
                  ...
                  className={`w-full py-2.5 rounded-xl font-display text-[11px] font-black uppercase tracking-wider transition-all duration-150 active:scale-95 cursor-pointer mt-4 ${
                    isElevated 
      ...
      <div className="space-y-4">
        <div className="flex items-center space-x-2 pb-1.5 border-b border-slate-200">
          ...
          <h2 className="text-sm font-black text-[#0A3D91] uppercase tracking-wider">
            {language === 'am' ? 'ቪአይፒ ከፍተኛ የኢንቨስትመንት ደረጃዎች (VIP Levels)' : 'VIP Premium Investment Levels'}
          </h2>
        </div>
        <p className="text-[11px] text-slate-500 font-bold -mt-2">
          {language === 'am' ? 'የቪአይፒ ደረጃዎችን ለማንቃት የብሔራዊ መታወቂያ (KYC) መረጋገጥ አለበት።' : 'Account verification (KYC) is required only for VIP levels.'}
        </p>
