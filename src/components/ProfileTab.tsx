    ...
    <div className="space-y-6 pb-28 animate-in fade-in slide-in-from-bottom-4 duration-300">
      ...
      <div className="p-6 rounded-[2.2rem] bg-white border border-slate-100 shadow-sm space-y-5">
        ...
        <div className="space-y-4 pt-2">
          ...
          {/* Gamified Referral Leaderboard Section (Seeded Communities and Real Stats) */}
          <div className="pt-4 border-t border-slate-150/60 font-sans text-left space-y-3">
            <div className="flex items-center space-x-1.5 text-slate-800">
              <Trophy className="w-4 h-4 text-amber-500 fill-amber-400/20" />
              <span className="text-[10px] font-sans font-black uppercase tracking-widest text-[#0A3D91]">
                {language === 'am' ? 'የግብዣ መሪዎች ሰሌዳ' :
                 language === 'om' ? 'Hordoffii Miltoo Ol\'aanoo' :
                 language === 'ti' ? 'ዕውት መሪሒነት ሰሌዳ' :
                 language === 'so' ? 'Kala Horaynta Casuumayasha' :
                 'Referral Leaderboard'}
              </span>
            </div>

            {/* Rank Standing Card */}
            <div className="p-3.5 bg-amber-50/40 rounded-2xl border-2 border-amber-200/60 flex items-center justify-between">
              <div>
                <span className="text-[8px] text-amber-800 block uppercase font-sans font-black tracking-wider leading-none">
                  {language === 'am' ? 'የእርስዎ ደረጃ' : 'Your Referral Standing'}
                </span>
                <span className="text-sm font-black text-slate-900 mt-1 block uppercase font-sans">
                  {profile.teamSize >= 5 ? (
                    'Rank #1'
                  ) : profile.teamSize >= 3 ? (
                    'Rank #3'
              ...
              <div className="text-right">
                <span className="text-[8px] text-slate-500 block uppercase font-black leading-none">
                  {t.teamSize}
                </span>
                <span className="text-xs font-black text-[#0A3D91] mt-1 block">
                  {profile.teamSize || 0} {language === 'am' ? 'አጋሮች' : 'partners'}
                </span>
              </div>
            </div>

            {/* Seeded Top 3 Partners for authentic community feel */}
            <div className="space-y-2 bg-slate-50/50 rounded-2xl border border-slate-100 p-2.5">
              <div className="flex items-center justify-between text-[8px] font-black text-slate-400 uppercase tracking-widest px-1">
                <span>{language === 'am' ? 'የመሪዎች ስም' : 'Top Team Leader'}</span>
                <span>{language === 'am' ? 'አባላት / ጉርሻ' : 'Partners / Rewards'}</span>
              </div>

              {/* Leader #1 */}
              <div className="flex items-center justify-between p-2 bg-white rounded-xl border border-slate-100 shadow-3xs">
                <div className="flex items-center space-x-2 min-w-0">
                  <div className="w-5 h-5 rounded-lg bg-yellow-400 text-slate-950 flex items-center justify-center text-[10px] font-black font-sans shadow-2xs">
                    👑
                  </div>
                  <div className="min-w-0">
                    <p className="text-[10px] font-black text-slate-900 leading-none">Abebe_K</p>
                    <p className="text-[8px] text-emerald-600 font-extrabold font-mono leading-none mt-0.5">Top Earner</p>
                  </div>
                </div>
                <div className="text-right shrink-0">
                  <p className="text-[10px] font-black text-slate-900 line-height-none">37 partners</p>
                  <p className="text-[8.5px] text-slate-500 font-extrabold font-mono mt-0.5 leading-none">245,390 ETB</p>
                </div>
              </div>
              ...
              <div className="flex items-center justify-between p-2 bg-white rounded-xl border border-slate-100 shadow-3xs">
                <div className="flex items-center space-x-2 min-w-0">
                  <div className="w-5 h-5 rounded-lg bg-slate-300 text-slate-800 flex items-center justify-center text-[9px] font-bold font-sans">
                    🥈
                  </div>
                  <div className="min-w-0">
                    <p className="text-[10px] font-black text-slate-900 leading-none">Selam_T</p>
                    <p className="text-[8px] text-[#0A3D91] font-extrabold leading-none mt-0.5">Gold Partner</p>
                  </div>
                </div>
                <div className="text-right shrink-0">
                  <p className="text-[10px] font-black text-slate-900">24 partners</p>
                  <p className="text-[8.5px] text-slate-500 font-extrabold font-mono mt-0.5 leading-none">158,110 ETB</p>
                </div>
              </div>
              ...
              <div className="flex items-center justify-between p-2 bg-white rounded-xl border border-slate-100 shadow-3xs">
                <div className="flex items-center space-x-2 min-w-0">
                  <div className="w-5 h-5 rounded-lg bg-orange-300 text-slate-800 flex items-center justify-center text-[9px] font-bold font-sans">
                    🥉
                  </div>
                  <div className="min-w-0">
                    <p className="text-[10px] font-black text-slate-900 leading-none">Desta_M</p>
                    <p className="text-[8px] text-slate-600 font-extrabold leading-none mt-0.5">Active Member</p>
                  </div>
                </div>
                <div className="text-right shrink-0">
                  <p className="text-[10px] font-black text-slate-900">18 partners</p>
                  <p className="text-[8.5px] text-slate-500 font-extrabold font-mono mt-0.5 leading-none">112,850 ETB</p>
                </div>
              </div>
            </div>
          </div>
