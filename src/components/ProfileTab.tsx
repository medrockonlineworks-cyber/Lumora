...
export default function ProfileTab({ 
  ...
  const handleNameChange = async () => {
    ...
    if (!newName.trim()) {
      setNameError(
        language === 'am' ? 'እባክዎ አዲሱን ሙሉ ስም ያስገቡ' : 'New name cannot be empty.'
      );
      return;
    }

    if (newName.trim().length < 2 || newName.trim().length > 64) {
      setNameError(
        language === 'am' ? 'ሙሉ ስም ከ2 እስከ 64 ቁምፊዎች መሆን አለበት' : 'Full name must be between 2 and 64 characters.'
      );
      return;
    }
    ...
    try {
      const response = await fetch('/api/profiles/change-name', {
        method: 'POST',
      ...
      if (!response.ok) {
        ...
        setNameSuccess(
          language === 'am' ? 'ሙሉ ስምዎ በተሳካ ሁኔታ ተቀይሯል!' : 'Your full name has been updated successfully!'
        );
        if (onRefresh) onRefresh();
      }
    } catch (err) {
      setNameError('Network error changing display name.');
    } finally {
    ...
    <div className="space-y-6 pb-28 animate-in fade-in slide-in-from-bottom-4 duration-300">
      ...
      <div className="rounded-[2.2rem] bg-white border border-slate-100 text-center relative overflow-hidden shadow-sm pb-6">
        ...
        <div className="px-6 space-y-1">
          ...
          {/* Admin-Registered Pending Password Notice */}
          {profile?.registeredByAdmin && !profile?.fullOwner && (
            <div className="mt-3 w-full max-w-sm p-3 bg-amber-50 rounded-2xl border border-amber-200 shadow-xs text-center space-y-1 animate-pulse">
              <span className="text-[8px] bg-amber-600 text-white font-black uppercase py-0.5 px-2 rounded-lg inline-block tracking-widest font-sans">
                {language === 'am' ? 'የይለፍ ቃል መቀየር ያስፈልጋል' : 'Ownership Action Required'}
              </span>
              <p className="text-[9.5px] text-amber-950 font-bold leading-normal">
                {language === 'am' ? 'መለያዎ በአስተዳዳሪ የተመዘገበ ነው። ሙሉ ባለቤትነት ለማግኘት እባክዎ ከዚህ በታች ያለውን ይለፍ ቃል ይቀይሩ።' : 
                 'Your account was registered by an administrator. Please update your secure login password below to assume full, private ownership.'}
              </p>
            </div>
          )}
          ...
          {profile?.fullOwner && (
            <div className="mt-2.5 flex items-center justify-center">
              <span className="inline-flex items-center space-x-1 px-2.5 py-0.5 rounded-full bg-blue-50 text-[#0180FE] border border-blue-150 text-[9px] font-extrabold uppercase tracking-widest font-sans">
                🛡️ {language === 'am' ? 'ሙሉ ባለቤት የተረጋገጠ' : 'Full Owner Registered'}
              </span>
            </div>
          )}
      ...
      <div className="p-3.5 bg-white border border-slate-200 rounded-[2rem] space-y-1 shadow-sm">
        ...
        <button
          id="profile-toggle-change-name-btn"
        ...
        >
          <div className="flex items-center space-x-3.5">
            <div className="p-2 bg-indigo-50 text-indigo-700 rounded-xl group-hover:bg-indigo-100 transition-all">
              <User className="w-4.5 h-4.5 text-indigo-700" />
            </div>
            <div className="text-left">
              <span className="block text-slate-950 text-[11.5px] font-black leading-none">
                {language === 'am' ? 'ሙሉ ስም ይለውጡ' : 'Change Full Name'}
              </span>
              <span className="text-[8.5px] text-indigo-800 block tracking-widest uppercase font-mono mt-1 font-extrabold">
                {language === 'am' ? 'የመለያ ሙሉ ስምዎን ያድሱ' : 'Modify user display name'}
              </span>
            </div>
          </div>
          <ChevronRight className={`w-4 h-4 text-slate-700 transition-transform duration-200 ${showNameForm ? 'rotate-90' : ''}`} />
        </button>
        ...
        {showNameForm && (
          <div className="p-4 bg-slate-50 border border-slate-100 rounded-2xl mt-2 space-y-4 font-sans text-left animate-in fade-in duration-200">
            <h4 className="font-display font-black text-[11px] text-[#0A3D91] uppercase tracking-widest border-b border-slate-200 pb-2 flex items-center space-x-1.5">
              <User className="w-3.5 h-3.5 text-[#0A3D91]" />
              <span>
                {language === 'am' ? 'ሙሉ ስምዎን ያሻሽሉ' : 'Modify Full Name'}
              </span>
            </h4>
            ...
            <form onSubmit={(e) => { e.preventDefault(); handleNameChange(); }} className="space-y-4">
              ...
              <div className="space-y-1">
                <label className="text-[8.5px] text-[#0A3D91] block uppercase font-sans font-black tracking-widest pl-1">
                  {language === 'am' ? 'አዲስ ሙሉ ስም' : 'New Full Name'}
                </label>
                <input
                  type="text"
                  value={newName}
                  onChange={(e) => setNewName(e.target.value)}
                  placeholder={language === 'am' ? 'አዲሱን ሙሉ ስም ያስገቡ' : 'Enter new full name'}
                  className="w-full px-3 py-2.5 bg-white border border-slate-200 rounded-xl text-[11px] font-bold text-slate-900 placeholder-slate-400 focus:outline-none focus:border-blue-400 transition-all"
                />
              </div>
              ...
              >
                {submittingName ? (
                  ...
                  <>
                    ...
                    <span>
                      {language === 'am' ? 'ስም ቀይር' : 'Change Name'}
                    </span>
                  </>
                )}
              </button>
